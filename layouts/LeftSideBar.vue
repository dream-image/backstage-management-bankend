<template>
  <div>
    <div id="all"
      style="background-color: rgba(128, 128, 128, 0.63); position:absolute; width: 100%; height: 100%;z-index: 10; display: none;">
    </div>
    <div class="common-layout">
      <el-container style="position: absolute;height: 100%;width: 100%;">
        <el-header id="header-wrapper">
          <div id="title-wrapper">
            <img src="~/assets/icon.svg" style="
              width: 20px;
              height: 20px;
              position: relative;
              margin-right: 10px;
          ...
          <div id="user-wrapper" @click="isShowLogOut = !isShowLogOut">
            ...
            <transition name="el-zoom-in-top">
              <el-button id="logout" v-show="isShowLogOut" type="primary" plain @click="logout"
                style="z-index: 1;">退出登陆</el-button>
            </transition>
          </div>
        </el-header>
        <el-container style="user-select: none;">
          <el-aside width="200px">
            <el-scrollbar>
              <el-menu :default-openeds="['1']" :unique-opened="true" :default-active="currentRoute">
                <el-sub-menu index="1">
                  <template #title>
                    <el-icon>
                      <HomeFilled color="#79bbff" />
                    </el-icon>题库管理
                  </template>
                  <el-menu-item-group>
                    <el-menu-item index="1-1" @click="goto(routeUrl.QUESTION)">题库总览</el-menu-item>
                  </el-menu-item-group>
...
async function logout() {
  localStorage.setItem('hasRefresh', false)
  await navigateTo("/login")
}
onMounted(() => {
  //当页面刷新加载回来的时候，保证左侧选择栏能够根据路由指定正确的激活状态
  if (new RegExp(routeUrl.QUESTION).test(path)) {
    currentRoute.value = "1-1";
  } else if (new RegExp(routeUrl.SELECTION).test(path)) {
    currentRoute.value = "1-2";
  } else if (new RegExp(routeUrl.USER).test(path)) {
